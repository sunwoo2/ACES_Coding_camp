                ...
                __local        float* restrict Bwrk)
{
    ...
    for (Kblk = 0;  Kblk<Num_BLK;  Kblk++)	// C(i,j)를 위해 A한블럭*B한블럭 64개 더해주는 loop 
    {
       ...
       Awrk[jloc*blksz+iloc] = A[Abase+jloc*N+iloc];	// A전체에서 한블럭을 그냥 한블럭으로 맵핑(local 행과 열이 바뀌긴 했지만 어차피 전체 다 병렬실행 하면 다 맵핑되는거니깐)
    ...
    C[j*N+i] = Ctmp;	// 대칭행렬구해서 마지막에 행,열 위치 바꾸는 원리?!?!
}
